...
trait MongoDbTaskRepository extends Repository[Task, MonadicResult] with Connection {
  ...
  override def insert(elt: Task): MonadicResult[Option[Task]] = AsyncResult(collection.save(elt).map { lastError => if (lastError.ok) Some(elt) else None })
